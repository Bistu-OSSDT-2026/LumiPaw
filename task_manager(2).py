...
#     "success": bool
# }
# 注意：金币奖励由经济系统根据 actual_duration 独立计算
...
def finish_task() -> Optional[dict]:
    ...
    返回 task_completed_event 事件字典（不包含金币奖励），
    金币由经济系统根据 actual_duration 独立计算。
    重置 current_task 为 None。
    ...
    # 构建事件字典（不包含 coins_reward，由经济系统计算）
...
if __name__ == "__main__":
    ...
    print(f"  注意：金币由经济系统根据 duration={event['duration']} 计算")
    print(f"  当前任务: {get_current_task()}")
    ...
    event = finish_task()
    print(f"  task_completed_event: {event}")
    print(f"  success=False (时长不足)")

    print("\n" + "=" * 60)
    print("测试完成 - 金币由经济系统根据实际时长计算")
    print("=" * 60)
